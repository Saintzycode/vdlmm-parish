...
const links = [
  ...
  { label: 'Announcements', to: { path: '/', hash: '#announcements' } },
  { label: 'Mass Schedule', to: { path: '/', hash: '#schedule' } },
  { label: 'Location', to: { path: '/', hash: '#location' } },
        ...
        <div class="absolute right-4 top-0 h-16 flex items-center z-50">
          <ul class="hidden lg:flex items-center gap-6 mr-4">
            <li v-for="navLink in links" :key="navLink.label">
              <router-link :to="navLink.to" class="text-[0.72rem] font-bold tracking-[0.12em] uppercase text-white/60 hover:text-[#E4C876] transition-colors duration-200">{{ navLink.label }}</router-link>
            </li>
            <li v-if="isAdmin">
            ...
            <li>
              <router-link to="/" class="text-[0.72rem] font-bold tracking-[0.12em] uppercase bg-[#C8A84B] text-[#0D2340] px-4 py-2 rounded-sm hover:bg-[#E4C876] transition-colors duration-200">Visit Us</router-link>
            </li>
          </ul>
    ...
    <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-2">
      <div v-if="isOpen" class="lg:hidden bg-[#0D2340] border-t border-[#C8A84B]/20">
        <ul class="flex flex-col px-4 py-4 gap-1">
          <li v-for="navLink in links" :key="navLink.label">
            <router-link :to="navLink.to" class="block py-3 px-3 text-sm font-semibold tracking-widest uppercase text-white/70 hover:text-[#E4C876] hover:bg-white/5 rounded transition-colors duration-200" @click="closeMenu">{{ navLink.label }}</router-link>
          </li>
          <li v-if="isAdmin">
          ...
          <li class="mt-2">
            <router-link to="/" class="block py-3 px-3 text-sm font-bold tracking-widest uppercase text-center bg-[#C8A84B] text-[#0D2340] rounded-sm hover:bg-[#E4C876] transition-colors duration-200" @click="closeMenu">Visit Us</router-link>
          </li>
        </ul>
      </div>
    </transition>
